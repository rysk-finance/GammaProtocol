...
contract('OTCWrapper', ([admin, beneficiary, keeper, random]) => {
  ...
  // OTC Wrapper module
  let otcWrapperImplementation: OTCWrapperV2Instance
  ...
  // Prices
  const usdcStartingPrice = 0.9 // Since USDC is never exactly 1, we set it at 0.9 to start to exaggerate the calculation differences
  const wethStartingPrice = 1500
  const wbtcStartingPrice = 20000
  const usdcExpiryPrice = 1.1 // Similar to above, since USDC is never exactly 1, we set it at 1.1 to expiry to exaggerate the calculation differences
  ...
  before('Deployment', async () => {
    ...
    // deploy OTC wrapper
    otcWrapperImplementation = await OTCWrapperV2.new(minimalForwarder.address, usdc.address)
    ...
    otcWrapperProxy.initialize(addressBook.address, admin, stableStrikeAsset.address, 15 * 60)

    // set OTC wrapper address in addressbook
    await addressBook.setOTCWrapper(otcWrapperProxy.address)

    // set oracle price
    await oracle.setRealTimePrice(usdc.address, scaleBigNum(usdcStartingPrice, 8))
    await oracle.setRealTimePrice(stableStrikeAsset.address, scaleBigNum(1, 18))
    await oracle.setRealTimePrice(weth.address, scaleBigNum(wethStartingPrice, 8))
    await oracle.setRealTimePrice(wbtc.address, scaleBigNum(wbtcStartingPrice, 8))

    // set expiry for options
    expiry = createValidExpiry(Number(await time.latest()), 10)

    // USDC and stableStrikeAsset at expiry
    await oracle.setExpiryPriceFinalizedAllPeiodOver(usdc.address, expiry, createTokenAmount(usdcExpiryPrice), true)
    await oracle.setExpiryPriceFinalizedAllPeiodOver(stableStrikeAsset.address, expiry, createTokenAmount(1), true)

    // admin whitelists product and collateral
    await whitelist.whitelistCollateral(usdc.address)
    await whitelist.whitelistProduct(weth.address, stableStrikeAsset.address, usdc.address, false)
    await whitelist.whitelistNakedCollateral(usdc.address, weth.address, false)

    await whitelist.whitelistProduct(weth.address, stableStrikeAsset.address, wbtc.address, true)
    ...
    await calculator.setSpotShock(weth.address, stableStrikeAsset.address, usdc.address, false, scaleBigNum(1500, 35))
    ...
    await calculator.setSpotShock(weth.address, stableStrikeAsset.address, wbtc.address, true, scaleBigNum(1500, 35))
  })
  ...
  describe('permit setup', () => {
    it('set up user permit USDC signature 1', async () => {
      ...
      const value = parseUnits(premiumAmountInUSD, 6).toNumber()
      const nonce = 0
  ...
  describe('#initialize', () => {
    it('should revert if initialized with 0 USDC address', async () => {
      await expectRevert(OTCWrapperV2.new(minimalForwarder.address, ZERO_ADDR), 'OTCWrapper: usdc address cannot be 0')
    })
    it('should revert if initialized with 0 addressBook address', async () => {
      const otcWrapper = await OTCWrapperV2.new(minimalForwarder.address, usdc.address)
      await expectRevert(
        otcWrapper.initialize(ZERO_ADDR, beneficiary, stableStrikeAsset.address, new BigNumber(15 * 60)),
    ...
    it('should revert if initialized with 0 beneficiary address', async () => {
      const otcWrapper = await OTCWrapperV2.new(minimalForwarder.address, usdc.address)
      await expectRevert(
        otcWrapper.initialize(addressBook.address, ZERO_ADDR, stableStrikeAsset.address, new BigNumber(15 * 60)),
        'OTCWrapper: beneficiary address cannot be 0',
      )
    })
    it('should revert if initialized with 0 strike asset address', async () => {
      const otcWrapper = await OTCWrapperV2.new(minimalForwarder.address, usdc.address)
      await expectRevert(
        otcWrapper.initialize(addressBook.address, beneficiary, ZERO_ADDR, new BigNumber(15 * 60)),
        'OTCWrapper: strike asset address cannot be 0',
      )
    })
    it('should revert if initialized with 0 fill deadline', async () => {
      const otcWrapper = await OTCWrapperV2.new(minimalForwarder.address, usdc.address)
      await expectRevert(
        otcWrapper.initialize(addressBook.address, random, stableStrikeAsset.address, new BigNumber(0)),
  ...
  describe('Set strike asset', () => {
    it('should revert if initialized with 0 strike address', async () => {
      await expectRevert(otcWrapperProxy.setStrikeAsset(ZERO_ADDR), 'OTCWrapper: strike asset address cannot be 0')
    })
    it('should revert if caller is not the owner', async () => {
      await expectRevert(otcWrapperProxy.setStrikeAsset(random, { from: random }), 'Ownable: caller is not the owner')
    })
    it('sucessfully sets strike address to an arbitrary non-zero address (representing an ERC20 token)', async () => {
  ...
  describe('Set max deviation', () => {
    ...
    it('should revert if max deviation is higher than 100%', async () => {
      await expectRevert(
        otcWrapperProxy.setMaxDeviation(weth.address, 10001),
        'OTCWrapper: max deviation should not be higher than 100%',
      )
    })
  ...
  describe('Refresh configuration', () => {
    it('should revert if caller is not the owner', async () => {
      await expectRevert(otcWrapperProxy.refreshConfiguration({ from: random }), 'Ownable: caller is not the owner')
    })
  ...
  describe('Place order', () => {
    ...
    it('user sucessfully creates a new order via direct call', async () => {
      ...
      const size = 100
      const parsedSize = parseUnits('100', 8)
      const notional = (size * wethStartingPrice).toString()
      ...
      const tx = await otcWrapperProxy.placeOrder(
        ...
        strikePrice,
        expiry,
        parseUnits(premiumAmountInUSD.toString(), 6),
        parsedSize,
        {
          from: user,
        },
      ...
      assert.equal((await otcWrapperProxy.orders(1))[5].toString(), parseUnits(premiumAmountInUSD, 6).toString())
      assert.equal((await otcWrapperProxy.orders(1))[6].toString(), parseUnits(notional, 6).toString())
      ...
      assert.equal((await otcWrapperProxy.orders(1))[12].toString(), parsedSize.toString())
      ...
      assert.equal(logs[0].args.premium.toString(), parseUnits(premiumAmountInUSD, 6).toString())
      assert.equal(logs[0].args.size.toString(), parsedSize.toString())
      assert.equal(logs[0].args.buyer.toString(), user)
      assert.equal(logs[0].args.notional.toString(), parseUnits(notional, 6).toString())
    })
  ...
  describe('Upgrade contract to new minimal forwarder', () => {
    it('successfully upgrades the contract to new minimal forwarder and new USDC address', async () => {
      ...
      const newOTCWrapperImplementation = await OTCWrapperV2.new(newMinimalForwarder.address, random)
