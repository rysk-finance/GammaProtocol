...
contract('Short Put Spread Option closed before expiry flow', ([accountOwner1, nakedBuyer, accountOwner2]) => {
  ...
  describe('Integration test: Open a short put spread and close it before expiry', () => {
    ...
    it('accountOwner1 should be able to open a short put spread', async () => {
      ...
      assert.equal(vaultAfter[0].shortOtokens[0], higherStrikePut.address, 'Incorrect short otoken in the vault')
      assert.equal(vaultAfter[0].collateralAssets[0], usdc.address, 'Incorrect collateral asset in the vault')
      assert.equal(vaultAfter[0].longOtokens[0], lowerStrikePut.address, 'Incorrect long otoken in the vault')
