...
contract('Long Call Spread Option expires Otm flow', ([accountOwner1, nakedBuyer, accountOwner2]) => {
  ...
  describe('Integration test: Close a long call spread collateralised with usdc after it expires OTM', () => {
    ...
    it('nakedBuyer: redeem the higher strike OTM call option after expiry', async () => {
...
contract('Long Call Spread Option expires Otm flow swap high strike and low strike', ([accountOwner1, nakedBuyer, accountOwner2]) => {
  let expiry: number
  let scaledUnderlyingPrice: BigNumber
  let scaledCollateralAmount: BigNumber
  let spreadCollatAmount: string
  let addressBook: AddressBookInstance
  let calculator: MarginCalculatorInstance
  let controllerProxy: ControllerInstance
  let controllerImplementation: ControllerInstance
  ...
  describe('Integration test: Close a long call spread collateralised with usdc after it expires OTM', () => {
    ...
    before(
      ...
      async () => {
        ...
        // accountOwner2 transfers their lower strike Call option to accountOwner1
        await lowerStrikeCallUSD.transfer(accountOwner1, scaledOptionsAmount, { from: accountOwner2 })
        spreadCollatAmount = createTokenAmount((lowerStrike - higherStrike) * optionsAmount, 6)
        const actionArgsAccountOwner1 = [
          {
            actionType: ActionType.OpenVault,
            owner: accountOwner1,
    ...
    it('accountOwner1: close an OTM long call spread position after expiry', async () => {
      ...
      assert.equal(
        new BigNumber(vaultStateBeforeSettlement[0]).toString(),
        vaultBefore[0].collateralAmounts[0].toString(),
      )
      assert.equal(vaultStateBeforeSettlement[1], true)
      ...
      // check balances before and after changed as expected
      assert.equal(
        ownerUsdcBalanceBefore.plus(createTokenAmount(collateralPayout, usdcDecimals)).plus(spreadCollatAmount).toString(),
        ownerUsdcBalanceAfter.toString(),
        'usdc balance mismatch',
      )
      assert.equal(
        marginPoolUsdcBalanceBefore.minus(createTokenAmount(collateralPayout,usdcDecimals)).minus(spreadCollatAmount).toString(),
        marginPoolUsdcBalanceAfter.toString(),
        'pool usdc balance mismatch',
      )
      assert.equal(ownerShortOtokenBalanceBefore.toString(), ownerShortOtokenBalanceAfter.toString())
